    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (action != null && action.equals("0")) {
                ...
                //判断是否已经绑定过了会员卡  Is_WXBind 字符串类型 0 未绑定 1  已经绑定
                PreparedStatement pasts = conn.prepareStatement("select * from t_vip where Is_WXBind<>? and  (cVipno=? or cWeixinId=? )");
                pasts.setString(1, "0");
                pasts.setString(2, cTel);
                pasts.setString(3, openid);
                ResultSet rs = pasts.executeQuery();
                if (rs.next()) {
                    DB.closeResultSet(rs);
                    DB.closePreparedStatement(pasts);
                    //response.sendRedirect("yizhuce.jsp");
                    RequestResult requestResult = new RequestResult(false, 2, "已经绑定过了", "bindFail.jsp");
                    out.print(requestResult.toJSON());
                    return;
                }
                ...
                sql = " update t_vip set Is_WXBind='1',cVipName=? ,cWorkUnit=? ,cWeixinId=? ,fcurvalue=fcurvalue+?,fCurValue_Pos=fCurValue_Pos+? where cVipno=?";
